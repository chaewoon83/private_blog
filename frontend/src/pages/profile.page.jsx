        ...
        <AnimationWrapper>
            {
                loading ? <Loader /> : 
                    ...
                    // if over md screen item align from right->left
                    <section className="min-h-[calc(100vh-80px)] md:flex flex-row-reverse items-start gap-5 min-[1100px]:gap-12">
                        ...
                        <div className="flex flex-col max-md:items-center gap-5 min-w-[250px] md:w-[50%] md:pl-8 md:border-l border-grey md:top-[100px] md:py-10">
                            {/* profile infos */}
                            <img src={profile_img} className="w-48 h-48 bg-grey rounded-full md:w-32 md:h-32"></img>

                            <h1 className="text-2xl font-medium"> @{profile_username}  </h1>
                            <p className="text-xl capitalize h-6"> {fullname}</p>
                            <p> {total_posts.toLocaleString()} Blogs - {total_reads.toLocaleString()} Reads</p>

                            {/* show edit profile button when profileid == loginid */}
                            ...
                            <AboutUser className=
                                "max-md:hidden" bio={bio} social_links={social_links} joinedAt={joinedAt} />
                        </div>
                        ...
                        <div className="max-md:mt-12 w-full">
                            <InPageNavigation routes={["Blogs published", "About"]} defaultHidden={["About"]}>
                                ...
                                {/* about us appears in small screen */}
                                <>
